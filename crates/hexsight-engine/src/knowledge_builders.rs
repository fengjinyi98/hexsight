...
impl ChampionCapabilityBuilder {
    ...
    pub fn build(&self, hero: &HeroData) -> ChampionCapability {
        ...
        // 羁绊
        let traits: Vec<String> = hero.species.split('|')
    ...
    pub fn build_all(&self, heroes: &[&HeroData]) -> (Vec<ChampionCapability>, CoverageReport) {
        ...
        for hero in heroes.iter().filter(|h| h.cost > 0 && !h.name.contains("假人")) {
        ...
        let report = CoverageReport {
            total: caps.len(),
            with_role: caps.iter().filter(|c| matches!(c.role, ChampionRole::PrimaryCarry | ChampionRole::SecondaryCarry)).count(),
    ...
    fn classify_damage_type(&self, desc: &str, hero: &HeroData) -> String {
        for kw in self.keywords.damage_types.get("true_damage").unwrap_or(&vec![]) {
            if desc.contains(kw) { return "true_damage".into(); }
        }
        let has_ad = self.keywords.damage_types.get("ad").unwrap_or(&vec![]).iter().any(|k| desc.contains(k));
        let has_ap = self.keywords.damage_types.get("ap").unwrap_or(&vec![]).iter().any(|k| desc.contains(k));
        if has_ad && has_ap { return "hybrid".into(); }
        if has_ap { return "ap".into(); }
        if has_ad { return "ad".into(); }
        if hero.skillBriefValue.contains("%") || hero.skillDesc.contains("法术") { return "ap".into(); }
        "ad".into()
    }
    ...
    fn classify_attack_patterns(&self, desc: &str) -> Vec<String> {
        ...
        if patterns.is_empty() { patterns.push("single_target".into()); }
        patterns
    }
    ...
    fn classify_cast_pattern(&self, desc: &str, hero: &HeroData) -> String {
        ...
        if init_mp as f64 / max_mp.max(1) as f64 > 0.5 { return "mana_cast".into(); }
        "mana_cast".into()
    }

    fn classify_scaling_stats(&self, desc: &str, hero: &HeroData) -> Vec<String> {
        let mut stats = Vec::new();
        if desc.contains("攻击力") || desc.contains("攻击") { stats.push("attack_damage".into()); }
        if desc.contains("法术强度") || desc.contains("法强") { stats.push("ability_power".into()); }
        if desc.contains("攻速") || desc.contains("攻击速度") { stats.push("attack_speed".into()); }
        let max_mp = hero.maxMP.parse::<i32>().unwrap_or(100);
        if max_mp >= 60 { stats.push("mana".into()); }
        if hero.criticalStrikeChance.parse::<i32>().unwrap_or(0) > 25 { stats.push("crit".into()); }
        if desc.contains("生命值") || desc.contains("护甲") { stats.push("health".into()); }
        if stats.is_empty() { stats.push("attack_damage".into()); }
        stats
    }
    ...
    fn classify_position(&self, hero: &HeroData) -> String {
        ...
        let hp = hero.initHP.parse::<i32>().unwrap_or(500);
        if range >= 3 { return "backline".into(); }
        if hp > 700 { return "frontline".into(); }
        "flex".into()
    }

    fn classify_item_strictness(&self, hero: &HeroData) -> String {
        if hero.cost >= 4 { return "high".into(); }
        if hero.cost >= 3 { return "medium".into(); }
        "low".into()
    }

    fn classify_power_spikes(&self, hero: &HeroData) -> Vec<PowerSpike> {
        vec![
            PowerSpike { spike_type: "star".into(), value: 2, impact: if hero.cost <= 2 { 30 } else { 15 } },
            PowerSpike { spike_type: "star".into(), value: 3, impact: if hero.cost <= 3 { 40 } else { 25 } },
            PowerSpike { spike_type: "item_count".into(), value: 2, impact: 20 },
        ]
    }

    fn classify_risks(&self, desc: &str) -> Vec<String> {
        let mut risks = Vec::new();
        if desc.contains("眩晕") || desc.contains("击飞") { risks.push("crowd_control".into()); }
        if desc.contains("爆发") { risks.push("burst_damage".into()); }
        if desc.contains("重伤") || desc.contains("减疗") { risks.push("anti_heal".into()); }
        risks
    }
    ...
    fn estimate_confidence(&self, desc: &str) -> f64 {
        ...
        for kws in self.keywords.damage_types.values() {
            if kws.iter().any(|k| desc.contains(k)) { hits += 1; }
        }
...
impl ItemValueBuilder {
    ...
    pub fn build(&self, equip: &EquipmentData) -> ItemValueProfile {
        ...
        // 伤害类型适配
        let damage_type_fit: Vec<String> = if equip.equip_type.contains("攻击") || desc_lower.contains("攻") {
            vec!["ad".into()]
        } else if desc_lower.contains("法") || desc_lower.contains("ap") {
            vec!["ap".into()]
        } else if desc_lower.contains("生命") || desc_lower.contains("护甲") || desc_lower.contains("抗") {
            vec!["tank".into()]
        } else {
            vec!["ad".into(), "ap".into()]
        };
        ...
        // 冲突组
        let conflict_group = self.keywords.conflict_groups.iter()
        ...
        // 阶段偏好
        let stage_bias = if equip.is_completed { "mid_late".into() } else { "early".into() };
    ...
    fn classify_best_for(&self, _desc: &str, damage_fit: &[String]) -> Vec<String> {
        ...
        for dt in damage_fit {
            if let Some(fits) = self.keywords.damage_type_item_fit.get(dt) {
                for f in fits { profiles.push(f.clone()); }
            }
        }
        profiles
    }
    ...
    fn extract_stats(&self, equip: &EquipmentData) -> Vec<ItemStat> {
        ...
        if desc.contains("攻击") || desc.contains("物理") {
            stats.push(ItemStat { name: "ad".into(), value: 10.0 });
        }
        if desc.contains("法强") || desc.contains("法术") || desc.contains("ap") {
            stats.push(ItemStat { name: "ap".into(), value: 10.0 });
        }
        if desc.contains("攻速") {
            stats.push(ItemStat { name: "attack_speed".into(), value: 10.0 });
        }
        if desc.contains("生命") || desc.contains("hp") {
            stats.push(ItemStat { name: "hp".into(), value: 150.0 });
        }
        if desc.contains("护甲") {
            stats.push(ItemStat { name: "armor".into(), value: 20.0 });
        }
        if desc.contains("魔抗") {
            stats.push(ItemStat { name: "mr".into(), value: 20.0 });
        }
        if desc.contains("暴击") {
            stats.push(ItemStat { name: "crit".into(), value: 10.0 });
        }
        stats
    }
}
    ...
    pub fn load(path: &std::path::Path) -> hexsight_core::HexResult<KnowledgeKeywords> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| hexsight_core::HexError::Config(format!(
                "读取关键词配置失败 {}: {}", path.display(), e
            )))?;
        let keywords: KnowledgeKeywords = serde_json::from_str(&content)
            .map_err(|e| hexsight_core::HexError::Config(format!(
                "解析关键词配置失败 {}: {}", path.display(), e
            )))?;
        Ok(keywords)
    }

    pub fn load_or_default(config_root: &std::path::Path, version: &str) -> KnowledgeKeywords {
        let path = config_root.join("rules").join(version).join("knowledge_keywords.json");
...
impl AugmentEffectProfileBuilder {
    pub fn new(keywords: KnowledgeKeywords) -> Self { Self { keywords } }
    ...
    pub fn build(&self, hex: &HexData) -> AugmentEffectProfile {
        ...
        // 经济标签
        if desc.contains("金币") || desc.contains("经济") || desc.contains("利息") || desc.contains("免费") {
            tags.push("generic_econ".into());
            hints.push("经济收益".into());
        }
        // 装备标签
        if desc.contains("装备") || desc.contains("散件") || desc.contains("锻造") || desc.contains("重铸") {
            tags.push("generic_item".into());
        }
        // 战力标签
        if desc.contains("攻击力") || desc.contains("法强") || desc.contains("伤害") || desc.contains("攻速") {
            tags.push("generic_combat".into());
        }
        // 羁绊标签
        if desc.contains("纹章") || desc.contains("转职") || desc.contains("之心") || desc.contains("羁绊") {
        ...
        if tags.is_empty() { tags.push("generic_combat".into()); }
        ...
        let lock_risk = if tags.contains(&"hero_commit".to_string()) { 60 } else { 20 };
...
impl TraitEffectProfileBuilder {
    pub fn build(&self, trait_data: &TraitData) -> TraitEffectProfile {
        let desc = format!("{} {} {}", trait_data.name, trait_data.desc, trait_data.prefix);
        let mut effect_tags = Vec::new();
        let mut lineup_value = 50;

        if desc.contains("伤害") || desc.contains("攻击") { effect_tags.push("damage".into()); lineup_value += 10; }
        if desc.contains("生命") || desc.contains("护甲") || desc.contains("魔抗") { effect_tags.push("defense".into()); lineup_value += 10; }
        if desc.contains("金币") || desc.contains("经济") { effect_tags.push("economy".into()); }
        if desc.contains("召唤") || desc.contains("分身") { effect_tags.push("summon".into()); }
        if desc.contains("控制") || desc.contains("眩晕") || desc.contains("击飞") { effect_tags.push("cc".into()); }
        if desc.contains("装备") || desc.contains("锻造") { effect_tags.push("item_synergy".into()); }

        TraitEffectProfile {
            trait_id: trait_data.checkId.clone(),
            name: trait_data.name.clone(),
            trait_type: if trait_data.trait_type == 0 { "race".into() } else { "job".into() },
...
impl ManualOverrideLoader {
    pub fn load(path: &std::path::Path) -> hexsight_core::HexResult<Vec<ManualOverride>> {
        ...
        let overrides: Vec<ManualOverride> = serde_json::from_str(&content)
            .map_err(|e| hexsight_core::HexError::Config(format!(
    ...
    /// 按类型和 ID 索引覆写
    pub fn index(overrides: &[ManualOverride]) -> std::collections::HashMap<String, &ManualOverride> {
        overrides.iter().map(|o| (format!("{}:{}", o.override_type, o.id), o)).collect()
    }
}
...
mod tests {
    ...
    fn make_hero(name: &str, cost: i32, hp: i32, ad: i32, skill: &str) -> HeroData {
        HeroData {
            id: format!("hero_{}", name), name: name.into(), price: cost.to_string(),
            picture: "".into(),
            skillName: "测试".into(), skillDesc: skill.into(),
            skillIcon: "".into(), skillBriefValue: "100%".into(), skillValueDesc: "".into(),
            species: "1".into(), hero_class: "2".into(),
            initHP: hp.to_string(), initAttackDamage: ad.to_string(),
            attackSpeed: "0.7".into(), armor: "30".into(),
            magicResist: "30".into(), attackRange: "1".into(),
            initMP: "0".into(), maxMP: "80".into(), criticalStrikeChance: "25".into(),
            cost, base_key: "".into(), star_level: 1,
        }
    }

    fn make_equip(id: &str, name: &str, etype: &str, desc: &str) -> EquipmentData {
        EquipmentData {
            id: id.into(), name: name.into(), equip_type: etype.into(),
            picture: "".into(), basicDesc: desc.into(), desc: "".into(),
            synthesis1: "0".into(), synthesis2: "0".into(), icon: "".into(),
            is_component: etype == "基础装备", is_completed: etype == "成型装备",
        }
    }
    ...
    fn build_tank_capability() {
        ...
        let cap = builder.build(&hero);
        assert!(matches!(cap.role, ChampionRole::MainTank | ChampionRole::OffTank));
    ...
    fn item_value_effect_tags() {
        ...
        let equip = make_equip("2001", "日炎斗篷", "成型装备", "每2秒灼烧周围敌人并施加重伤");
        let profile = builder.build(&equip);
        assert!(profile.effect_tags.iter().any(|t| t == "burn" || t == "anti_heal"));
    }
    ...
    fn real_data_knowledge_base_all_modes() {
        ...
        for mode in &["17", "16", "4"] {
            ...
            let kb = KnowledgeBaseBuilder::with_defaults()
                .build_all(&heroes, &equips, &hexes, &traits);
    ...
    fn manual_override_loader_empty_file() {
        let root = config_root();
        let path = root.join("rules").join("S18.1").join("manual_overrides.json");
