...
impl AugmentFitScorer {
    ...
    pub fn score(
        ...
        candidate_augments.iter().map(|(aug_id, aug_name)| {
            let mut reasons = Vec::new();
            let mut tags = Vec::new();
            let mut supported_ids = Vec::new();

            // 计算阵容覆盖度
            let coverage = profiles.iter()
                .filter(|p| {
            ...
            let lineup_coverage = (coverage as f64 / profiles.len().max(1) as f64 * 100.0) as i32;

            // 锁方向风险评估
            let lock_risk = if supported_ids.len() <= 1 {
                if is_first_augment { 75 } else { 40 }
            } else if supported_ids.len() <= 3 {
                if is_first_augment { 45 } else { 20 }
            } else {
                15
            };

            // 玩法开关价值
            let playstyle_value = if coverage >= 2 { 70 }
                else if coverage == 1 { 50 }
                else { 10 };

            // 通用战力（根据当前局势判断）
            let combat_power = if opening_is_strong { 60 }
                else if opening_is_weak { 40 }
                else { 50 };
    ...
    pub fn first_augment_decision(
        ...
        let should_lock = best.total_score >= 80
            && best.lock_risk < 40
            && best.supported_lineup_ids.len() <= 2;

        let mut recommended = format!("{} ({}分)", best.augment_name, best.total_score);
        if should_lock && !best.supported_lineup_ids.is_empty() {
            recommended.push_str(&format!(" [可锁: {}]", best.supported_lineup_ids.join(", ")));
        }
...
mod tests {
    ...
    fn make_profile(id: &str, rec_hex: &[&str], rep_hex: &[&str]) -> LineupProfile {
        LineupProfile {
            lineup_id: id.into(), name: format!("阵容{}", id), base_tier: 80,
            playstyle_tags: vec![PlaystyleTag::Standard],
            final_hero_ids: vec![], carry_hero_ids: vec![], tank_hero_ids: vec![],
            core_equipment_ids: vec![], tank_equipment_ids: vec![], equipment_order_ids: vec![],
            ...
            mode_specific: serde_json::Value::Null, carry_costs: vec![4],
            category: None,
        }
    }
    ...
    fn first_augment_prefers_flexibility() {
        ...
        let scores = AugmentFitScorer::score(
            &candidates, &profiles,
            &HashMap::new(), true, false, true,
        );
    ...
    fn first_augment_decision_outputs_lock_or_not() {
        let scores = vec![
            AugmentScore {
                augment_id: "hex_a".into(), augment_name: "海克斯A".into(),
                total_score: 85, combat_power: 70, lineup_coverage: 60,
                lock_risk: 25, playstyle_enable_value: 70,
                is_recommended: true,
