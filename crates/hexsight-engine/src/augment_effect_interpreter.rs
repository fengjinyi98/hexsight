...
impl AugmentEffectInterpreter {
    ...
    pub fn interpret(
        ...
        let rules: &[TagRules] = &[
            // 通用战力
            TagRules { keywords: &["攻击力", "法术强度", "法强", "ad", "ap", "伤害"], tag: "generic_combat", power: 25, lock: 0 },
            TagRules { keywords: &["攻速", "攻击速度", "暴击"], tag: "generic_combat", power: 20, lock: 0 },
            TagRules { keywords: &["生命值", "护甲", "魔抗", "双抗", "护盾", "治疗"], tag: "generic_combat", power: 15, lock: 0 },
            // 通用经济
            TagRules { keywords: &["金币", "经济", "利息", "免费"], tag: "generic_econ", power: 5, lock: 0 },
            TagRules { keywords: &["刷新", "商店", "d牌", "免费刷新"], tag: "generic_econ", power: 10, lock: 0 },
            TagRules { keywords: &["经验", "等级", "人口", "升星"], tag: "generic_econ", power: 10, lock: 0 },
            // 装备
            TagRules { keywords: &["装备", "散件", "锻造", "重铸", "拆卸", "组件"], tag: "generic_item", power: 15, lock: 0 },
            TagRules { keywords: &["成装", "神器", "奥恩", "光明"], tag: "generic_item", power: 25, lock: 10 },
            // 羁绊强化
            TagRules { keywords: &["羁绊", "纹章", "转职", "之心", "之魂", "之冕"], tag: "trait_commit", power: 20, lock: 30 },
            // 英雄专属
            TagRules { keywords: &["专属", "英雄强化"], tag: "hero_commit", power: 30, lock: 60 },
            // 赌狗开关
            TagRules { keywords: &["刷新", "复制", "免费"], tag: "reroll_enable", power: 5, lock: 20 },
            TagRules { keywords: &["低费", "1费", "2费", "三星"], tag: "reroll_enable", power: 0, lock: 30 },
            // 节奏/后期
            TagRules { keywords: &["连胜", "节奏", "前期", "开局"], tag: "tempo", power: 20, lock: 10 },
            TagRules { keywords: &["后期", "上限", "高费", "彩色"], tag: "late_cap", power: 5, lock: 10 },
            // 模式特殊
            TagRules { keywords: &["星神", "神明", "天选", "任务", "传奇"], tag: "mode_special", power: 10, lock: 40 },
            // 高风险
            TagRules { keywords: &["高风险", "赌", "概率", "随机"], tag: "high_risk", power: 10, lock: 30 },
        ];
        ...
        for rule in rules {
            for kw in rule.keywords {
                if combined.contains(kw) {
                    ...
                    lock_risk += rule.lock;
                    if rule.tag == "reroll_enable" { reroll_value += 15; }
                    break;
                }
            }
        }
        ...
        // 等级修正
        match augment_level {
            1 => { immediate_power -= 5; lock_risk -= 10; }
            3 => { immediate_power += 10; lock_risk += 10; }
            _ => {}
        }

        // 阵容覆盖度修正
        let lineup_coverage = (lineup_recommended_count as i32).min(5);
        if lineup_coverage >= 3 { lock_risk = (lock_risk as f64 * 0.6) as i32; }
        if lineup_coverage == 0 { lock_risk += 15; }
    ...
    pub fn interpret_all(
        ...
        candidates.iter().map(|(id, name, desc, level)| {
            let coverage = lineup_recommended_counts.iter()
                .find(|(aid, _)| aid == id)
                .map(|(_, c)| *c)
                .unwrap_or(0);
...
mod tests {
    ...
    fn interpret_generic_combat() {
        let effect = AugmentEffectInterpreter::interpret(
            "hex_001", "战斗强化", "获得30攻击力和20法术强度", 2, 3,
        );
    ...
    fn interpret_trait_commit() {
        let effect = AugmentEffectInterpreter::interpret(
            "hex_trait", "羁绊之魂", "获得一个随机纹章和羁绊强化", 3, 1,
        );
    ...
    fn interpret_reroll_enabler() {
        let effect = AugmentEffectInterpreter::interpret(
            "hex_reroll", "金色刷新", "每次D牌获得一次免费刷新，1费和2费英雄概率提升", 2, 2,
        );
    ...
    fn interpret_econ_augment() {
        let effect = AugmentEffectInterpreter::interpret(
            "hex_econ", "理财达人", "立即获得15金币，每回合额外获得2金币利息", 2, 0,
        );
