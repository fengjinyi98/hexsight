...
use hexsight_core::{
    AugmentsContext, EquipRef, EquipmentContext, HexRef, LineupCardData, LineupContextInfo,
    ModeContextInfo, ModeSpecificContext, RuleEquipmentSnapshot, RuleHeroSnapshot, RuleHexSnapshot,
    RulesContextOutput,
};
...
const SUPPORTED_MODES: &[(&str, &str, &str, &str, &[&str])] = &[
    (
        ...
        "S18",
        "m18",
        &["standardLineup", "godRewards", "transitionContacts"],
    ),
    (
        "16",
        "英雄联盟传奇",
        "S18",
        "m17",
        &["standardLineup", "unlockTasks", "transitionContacts"],
    ),
    (
        "4",
        "天选福星",
        "S18",
        "m17",
        &["standardLineup", "chosenMechanics", "transitionContacts"],
    ),
];
...
impl RulesContextBuilder {
    ...
    pub fn build(
        ...
        let resolved_equip_order =
            Self::resolve_equipment_order(&detail.equipment_order_ids, game_data);
        let resolved_recommended_hexes =
            Self::resolve_hexes(&detail.recommended_hex_ids, game_data);
        let resolved_replacement_hexes =
            Self::resolve_hexes(&detail.replacement_hex_ids, game_data);
        ...
        let augments = AugmentsContext {
            recommended: resolved_recommended_hexes
                .iter()
                .map(|h| HexRef {
                    id: h.id.clone(),
                    name: h.name.clone(),
                ...
                .collect(),
            replacement: resolved_replacement_hexes
                .iter()
                .map(|h| HexRef {
                    id: h.id.clone(),
                    name: h.name.clone(),
        ...
        let mode_specific = ModeSpecificContext {
            ...
            chosen: detail
                .chosen_contact
                .as_ref()
                .map(|c| serde_json::json!({ "id": c.id, "type": c.contact_type })),
        };
    ...
    pub fn mode_info(mode: &str) -> ModeContextInfo {
        SUPPORTED_MODES
            ...
            .map(
                |(id, name, season, _version_path, capabilities)| ModeContextInfo {
                    id: id.to_string(),
                    name: name.to_string(),
        ...
        pieces
            ...
            .map(|piece| {
                let (name, cost, picture) = if let Some(gd) = game_data {
                    ...
                    (
                        hero.map(|h| h.name.clone())
                            .unwrap_or_else(|| piece.hero_id.clone()),
                ...
                let equipment_names: Vec<String> = piece
                    .equipment_ids
                    .iter()
                    .map(|eid| {
                        game_data
                            .map(|gd| gd.equip_name(eid).to_string())
    ...
    fn resolve_hexes(ids: &[String], game_data: Option<&GameDataIndex>) -> Vec<RuleHexSnapshot> {
        ids.iter()
            .map(|id| {
